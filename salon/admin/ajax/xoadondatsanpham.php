<?php
	...
	if(isset($_GET['idddsp'])){ 

  

    $DB -> reset();
    $DB -> setTable('dondatsanpham');
	$DB->setWhere('iddondat',$_GET['idddsp']);
	if($DB->delete())
	{
		echo '<script>alert("Xóa thành công");</script>';
	}else {
		
		echo '<script>alert("Có liên kết bạn khôn thể xóa");</script>';
	}
	

    $DB -> reset();
    $DB -> setTable('chitietdondat');
    $DB->setWhere('iddondathangsanpham',$_GET['idddsp']);
	$DB->delete();
